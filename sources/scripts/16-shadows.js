function drawShadow() {
    ...
    shadowList.forEach(shadow => {
        ...
        // TODO: factoriser la distance de l'éclairage, le passer en constante...
        var activeDistance = isTorchLit ? LIGHT_LIT_RADIUS : LIGHT_RADIUS;

        gameContext.beginPath();
        if(distance < 20) {
            gameOver();
        } else if(distance < activeDistance - 5) {
            shadow.v = 1;
            var angle = Math.atan2(mapOffsetY + shadow.y - playerOffsetY, mapOffsetX + shadow.x - playerOffsetX) * 180 / Math.PI;
